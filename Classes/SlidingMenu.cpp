...
SlidingMenuGrid* SlidingMenuGrid::create(CCArray *items, int cols, int rows, CCPoint pos , CCPoint pad)
{ 
	return create(items,cols,rows,pos,pad,false);
}


SlidingMenuGrid* SlidingMenuGrid::create(CCArray *items, int cols, int rows, CCPoint pos , CCPoint pad, bool vertical)
{
...
void SlidingMenuGrid::ccTouchMoved(CCTouch* touch, CCEvent* event)
{
	...
	touchStop = touch->getLocation();
//    touchStop = CCDirector::sharedDirector()->convertToGL(touch->locationInView());
